...
static const float s_oo255 = 1.0f / 255.0f;
...
Quadtree::Quadtree(Rect _rect, int64_t _level, PageTable* pageTable)
    : m_rectangle(_rect)
    , m_level(_level)
...
void Quadtree::add(Page request, Point mapping) {
        ...
        for (int64_t i = 0; i < 4; ++i) {
            ...
            if (rect.contains({x, y})) {
                ...
                if (node->m_children[i] == nullptr) {
                    node->m_children[i] = BX_NEW(VirtualTexture::getAllocator(), Quadtree)(rect, node->m_level - 1, m_pageTable);
    ...
    m_pageTable->addToQueue({node->m_level, node->m_rectangle, node->m_mapping});
#endif
}
...
#if defined(USEPAGETABLECS)
        m_pageTable->addToQueue({node->m_level, node->getRectangle(index), node->m_mapping});
#endif
    }
}
...
// PageTable
PageTable::PageTable(PageCache* _cache, VirtualTextureInfo* _info, PageIndexer* _indexer, VirtualTextureShaders* _shaders)
    : m_info(_info)
    , m_indexer(_indexer)
    , m_shaders(_shaders)
    , m_quadtree(nullptr)
    , m_quadtreeDirty(true) // Force quadtree dirty on startup
{
    ...
    m_updateQueue.push_back({m_quadtree->m_level, m_quadtree->m_rectangle, m_quadtree->m_mapping});
#else
    ...
    _cache->added = [=](Page page, Point pt) {
        ...
        m_quadtree->add(page, pt);
    };
    _cache->removed = [=](Page page, Point pt) {
        m_quadtreeDirty = true;
        m_quadtree->remove(page);
        BX_UNUSED(pt);
    };
#if !defined(USEPAGETABLECS)
...
void PageTable::update(bgfx::ViewId viewId) {
    if (!m_quadtreeDirty) {
        return;
    }
    ...
    for (int64_t i = 0; i < PageTableSizeLog2; ++i) {
#if defined(USEPAGETABLECS)
        for (const PageTableQueue& queue : m_updateQueue) {
            writeQueueTexture(viewId, queue, m_texture, i);
        }
#else
        auto stagingTexture = m_stagingTextures[i];
        const auto size = uint16_t(m_info->GetPageTableSize() >> i);
        ...
        bgfx::blit(viewId, m_texture, uint8_t(i), 0, 0, 0, stagingTexture, 0, 0, 0, 0, size, size);
#endif
    }
#if defined(USEPAGETABLECS)
    m_updateQueue.clear();
...
#if defined(USEPAGETABLECS)
void PageTable::addToQueue(const PageTableQueue& queue) {
    m_updateQueue.push_back(queue);
}

void PageTable::writeQueueTexture(bgfx::ViewId viewId, const PageTableQueue& queue, bgfx::TextureHandle texture, int64_t miplevel) {
    if (queue.m_level < miplevel) {
        return;
    }
    const int64_t rx = queue.m_rectangle.m_x >> miplevel;
...
void PageLoader::copyColor(uint8_t* image, Page request) {
    ...
    {0, 0, 255, 255},   {0, 255, 255, 255},  {255, 0, 0, 255},   {255, 0, 255, 255},  {255, 255, 0, 255},  {64, 64, 192, 255},
    {64, 192, 64, 255}, {64, 192, 192, 255}, {192, 64, 64, 255}, {192, 64, 192, 255}, {192, 192, 64, 255}, {0, 255, 0, 255},
    };
...
bool PageCache::request(Page request, bgfx::ViewId viewId) {
    m_blitViewId = viewId;
...
TextureAtlas::TextureAtlas(VirtualTextureInfo* _info, int64_t _count, int64_t _uploadsperframe)
    ...
    m_texture = bgfx::createTexture2D((uint16_t)size, (uint16_t)size, false, 1, bgfx::TextureFormat::BGRA8, BGFX_SAMPLER_UVW_CLAMP | BGFX_TEXTURE_BLIT_DST);
}
...
void TextureAtlas::uploadPage(Point pt, uint8_t* data, bgfx::ViewId viewId) {
    ...
    bgfx::blit(viewId, m_texture, 0, xpos, ypos, 0, writer, 0, 0, 0, 0, pagesize, pagesize);
...
// VirtualTexture
VirtualTexture::VirtualTexture(TileDataFile* _tileDataFile, VirtualTextureInfo* _info, VirtualTextureShaders* _shaders, int64_t _atlassize, int _uploadsperframe, int64_t _mipBias)
    : m_tileDataFile(_tileDataFile)
    , m_info(_info)
    , m_shaders(_shaders)
...

{
    ...
    m_pageTable = BX_NEW(VirtualTexture::getAllocator(), PageTable)(m_cache, m_info, m_indexer, m_shaders);
...
void VirtualTexture::update(const tinystl::vector<int64_t>& requests, bgfx::ViewId viewId) {
    m_pagesToLoad.clear();
    ...
    if (touched < m_atlasCount * m_atlasCount) {
        ...
        bx::quickSort(m_pagesToLoad.begin(), uint32_t(m_pagesToLoad.size()), sizeof(vt::PageCount), [](const void* _a, const void* _b) -> int32_t {
        ...
        for (int64_t i = 0; i < loadcount; ++i)
            m_cache->request(m_pagesToLoad[i].m_page, viewId);
    } else {
...
TileDataFile::TileDataFile(const bx::FilePath& filename, VirtualTextureInfo* _info, int64_t pageCount, bool _readWrite)
    : m_info(_info)
    , m_pageCount(pageCount)
    , m_pageHeaders(nullptr)
    , m_fileOffset(0)
