...
public:
    ...
    void init(int32_t _argc, const char* const* _argv, uint32_t _width, uint32_t _height) override {
        ...
        for (uint16_t i = 0; i < 2; ++i) {
            bgfx::setViewClear(i, BGFX_CLEAR_COLOR | BGFX_CLEAR_DEPTH, i == 0 ? 0 : 0x101050ff, 1.0f, 0);
        }
        ...
        */
        ...
        m_vt = new vt::VirtualTexture(tileDataFile, m_vti, m_shaders, 4096, 1, 6);
    ...
    bool update() override {
        if (!entry::processEvents(m_width, m_height, m_debug, m_reset, &m_mouseState)) {
            imguiBeginFrame(m_mouseState.m_mx, m_mouseState.m_my,
                            (m_mouseState.m_buttons[entry::MouseButton::Left] ? IMGUI_MBUT_LEFT : 0) | (m_mouseState.m_buttons[entry::MouseButton::Right] ? IMGUI_MBUT_RIGHT : 0) |
            ...
            if ((BGFX_CAPS_TEXTURE_BLIT | BGFX_CAPS_TEXTURE_READ_BACK) != (bgfx::getCaps()->supported & (BGFX_CAPS_TEXTURE_BLIT | BGFX_CAPS_TEXTURE_READ_BACK))) {
                ...
                ImGui::SetNextWindowSize(ImVec2(m_width / 5.0f, m_height - 10.0f), ImGuiCond_FirstUseEver);
                ...
                ImGui::ImageButton(m_vt->getPageTableTexture(), ImVec2(m_width / 5.0f - 16.0f, m_width / 5.0f - 16.0f));
                if (!m_pageTableCaptureTextureFrame && ImGui::Button("Save PageTable")) {
                    if (m_pageTableCaptureTextureHandle.idx == bgfx::kInvalidHandle) {
                        const uint16_t size = (uint16_t)m_vti->GetPageTableSize();
                        m_pageTableCaptureTextureHandle =
                        bgfx::createTexture2D(size, size, true, 1, bgfx::TextureFormat::RGBA8, BGFX_SAMPLER_UVW_CLAMP | BGFX_SAMPLER_POINT | BGFX_TEXTURE_BLIT_DST | BGFX_TEXTURE_READ_BACK);
                        m_pageTableCaptureTextureData = bx::alloc(&m_vtAllocator, size * size * 4);
                    }
                    bgfx::blit(10, m_pageTableCaptureTextureHandle, 0, 0, m_vt->getPageTableTexture());
                    m_pageTableCaptureTextureFrame = bgfx::readTexture(m_pageTableCaptureTextureHandle, m_pageTableCaptureTextureData);
                }
            ...
            // Save page table image
            if (m_pageTableCaptureTextureFrame && frame == m_pageTableCaptureTextureFrame) {
                const int size = (int)m_vti->GetPageTableSize();
                stbi_write_png("pagetable.png", size, size, 4, m_pageTableCaptureTextureData, size * 4);
                m_pageTableCaptureTextureFrame = 0;
            }
    ...
    vt::VirtualTextureShaders* m_shaders;
    vt::VirtualTexture* m_vt;
    vt::FeedbackBuffer* m_feedbackBuffer;
    bgfx::TextureHandle m_pageTableCaptureTextureHandle = BGFX_INVALID_HANDLE;
    void* m_pageTableCaptureTextureData = nullptr;
    uint32_t m_pageTableCaptureTextureFrame = 0;
};

} // namespace

ENTRY_IMPLEMENT_MAIN(ExampleSVT, "40-svt", "Sparse Virtual Textures.", "https://bkaradzic.github.io/bgfx/examples.html#svt");
